...
globs: [".clinerules/**/*.md", "~/Documents/Cline/Rules/**/*.md"] # This rule is relevant when writing or editing any .clinerule
---
...
* Creating new Markdown files (`.md`) in the `.clinerules/` directory for project specific rules OR in the `~/Documents/Cline/Rules` durectory for global user rules.
    * Ask the user: "Should I create a workspace or user rule?"
        *   Provide options:
            *   "Workspace (`.clinerules` folder in the project)"
            *   "User (`~/Documents/Cline/Rules` folder)"
...
## 9. Compile rules into AGENTS.md for project
If user instructs to generate compile a single overview document for the project, create it as `.clinerules/AGENTS.md`.
If `CLAUDE.md` or `AGENTS.md` exists at the project root, take their contents as a starting point.

It should have a comprehensive guide for the project on a high level. Refer to individual files from the `memory-bank` folder if it exists.

IF `memory-bank` folder exists and not empty, then copy over the `memory-bank.md` rule from the user rules folder to `.clinerules` unless instructed otherwise.

The `.clinerules/AGENTS.md` file is equivalent of an `AGENTS.md`, or a `CLAUDE.md` at project root and if both exist, they might have equal content or have a symlink to each other.

Once the `.clinerules/AGENTS.md` file is ready, symlink it as the following files (delete them first if they exist):
- `AGENTS.md`
- `CLAUDE.md`
